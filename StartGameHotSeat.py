...
player_turn = 0  # 0 is player 1, 1 is player 2, value is updated every turn
board_list = [[None for _ in range(3)] for _ in range(3)]  # 2D list for playing board
bg_list = []  # list for background images
result, winner, winner_copy = None, None, None
global win_grid  # list of winning figures coordinates
...
class StartGameHotSeat(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.win_sound = mixer.Sound("audio/win.wav")

        # Process background image
        for _ in (glob("image/bg/*")):
            bg_list.append(_)  # looks for all images in the bg folder and adds them to a list
        self.bg_img = Image.open(random.choice(bg_list)).filter(
            ImageFilter.GaussianBlur(radius=0))  # 0 is no blur, can be changed
        self.bg_img = self.bg_img.resize((750, 500), Image.LANCZOS)
        ...
        self.board_img = self.board_img.resize((300, 300), Image.LANCZOS)
        ...
        self.circle_img = self.circle_img.resize((83, 83), Image.LANCZOS)
        self.circle_img = ImageTk.PhotoImage(self.circle_img)
        self.cross_img = Image.open("image/base/cross2.png")
        self.cross_img = self.cross_img.resize((83, 83), Image.LANCZOS)
        self.cross_img = ImageTk.PhotoImage(self.cross_img)

        # Define single square size on the playing board
        self.square_size = 300 / 3
        ...
        self.back_menu_icon = self.back_menu_icon.resize((16, 16), Image.LANCZOS).convert("RGBA")
        ...
        self.back_settings_icon = self.back_settings_icon.resize((16, 16), Image.LANCZOS).convert("RGBA")
        ...
        self.back_home_icon = self.back_home_icon.resize((16, 16), Image.LANCZOS).convert("RGBA")
    ...
    def process_click(self, event):
        ...
        square_size = self.square_size  # size of one of the squares on the board
        a = event.x  # x coordinate of mouse click
        b = event.y  # y coordinate of mouse click
        x = int((a - self.board_img_padx) / square_size)  # square x coordinate
        y = int((b - self.board_img_pady) / square_size)  # square y coordinate
        ...
        if x not in (0, 1, 2) or y not in (0, 1, 2) or board_list[x][y] is not None or b < self.board_img_pady or a < self.board_img_padx:
            return
        else:
            ...
            try:
                ...
                if result:
                    ...
                    mixer.Sound.play(self.win_sound)  # Play a sound effect
                    self.next_button.config(state="normal")  # Update NEXT button state
                    self.draw_win_grid(win_grid)  # Draw winning grid
    ...
    def draw_win_grid(self, win_grid, step_size=5):
        # Draw a line between the winning squares
